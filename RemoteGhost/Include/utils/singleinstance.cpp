...
// Description: Create shared memory mapped file or create view of it
{
	...
	// Set up data mapping
	mData = (LPTSTR)MapViewOfFile(mMap, FILE_MAP_WRITE, 0, 0, sizeof (TCHAR)* MAX_DATA_);
	...
	{
		// Clear data
		ZeroMemory(mData, sizeof (TCHAR)* MAX_DATA_);
	}
}
...
// Description: Set value in memory mapped file
{
	...
	if (lLock.IsLocked())
	{
		...
		if (_tcslen(aData) < MAX_DATA_)
		{
			// Copy data
			_tcscpy_s(mData, sizeof (TCHAR)* MAX_DATA_, aData);
		}
	}
}
...
// Name:        CSingleInstanceImpl 
// Type:        Constructor
// Description: Initialize member attributes
{
}
...
{
	//  [29-May-2015 HIEUTT] add try catch resource exception
	try
	{
		...
		if (mEvent)
		{
			...
			{
				// Create new event of singleton [2-June-2015 HIEUTT]
				//LogEvent(_T("Singleton create new event"));

				// Create event of thread synchronization, nameless local scope
				mSignal = new CEvent;
				if (mSignal)
				{
					// Create thread
	...
	// 		CString sLog(buf);
	// 
	// 		CLog::LogEvent(_T("Catch exception. Error:%s"), sLog);
	// 		return FALSE;
	// 	}
	//	END_CATCH	
	catch (...)
	{
		LogEvent(_T("Catch exception when start new instance."));
		return FALSE;
	}
}
