...
//IMPLEMENT_DYNAMIC(CCRC32,CObject)
...
// Static CRC table
DWORD CCRC32::s_arrdwCrc32Table[256] =
{
...
DWORD CCRC32::Crc32File(LPCTSTR szFilename, DWORD &dwCrc32)
{
	_ASSERTE(szFilename);
	...
	TRY
	{
		...
		else
		{
			...
			BYTE buffer[____CCRC32_MAX_BUFFER_SIZE];
			DWORD dwBytesRead;
	...
	CATCH_ALL(e)
	{
		...
		sLog.Format(_T("CRC32: Crc32File exception: %s"), GetWin32ErrorMessage(err));
		LOGERROR(sLog);
	}
	END_CATCH_ALL;
...
DWORD CCRC32::Crc32Bytes(BYTE* pbyte, DWORD bufferlength, DWORD &dwCrc32)
{
	_ASSERTE(pbyte);
	...
	CATCH_ALL(e)
	{
		dwErrorCode = ERROR_CRC;
		CString sLog(_T(""));
		sLog.Format(_T("CRC32: Crc32File exception: %s"), GetExceptionMessage(*e));
		LOGERROR(sLog);
	}
	END_CATCH_ALL;	
